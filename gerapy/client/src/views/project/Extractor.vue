<template>
	<div>
		<el-form-item>
			<h5 class="inline m-v-sm">{{ $lang.titles.callback }}</h5>
			<el-input
		...
		<el-form-item>
			<h5 class="inline m-v-sm">{{ $lang.titles.item }}</h5>
			<el-select v-model="extractor.item" :placeholder="$lang.titles.item"
			...
			<div v-if="Object.keys(extractor.attrs).length">
				<div v-for="(value, key, index) in extractor.attrs" :key="key">
					...
					<div v-for="(v, k, i) in value" :key="k" class="extractor-rule">
						<el-select v-model="v['method']" :placeholder="$lang.columns.method"
											 size="small"
